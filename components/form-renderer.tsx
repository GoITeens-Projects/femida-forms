...
export function FormRenderer({ form, onSubmit }: FormRendererProps) {
  ...
  const updateAnswer = (fieldId: string, value: string | boolean) => {
  ...
  const handleFileSelect = async (fieldId: string, file: File | null) => {
    ...
    if (file.size > 10 * 1024 * 1024) {
      setErrors((prev) => ({ ...prev, [fieldId]: 'File too large. Max 10MB.' }))
      return
    }
          ...
          <div className="space-y-2">
            ...
            {files[field.id] ? (
              <div className="flex items-center gap-2 rounded-md border bg-muted/50 p-3">
                <span className="flex-1 truncate text-sm">{files[field.id]?.name}</span>
                {uploading[field.id] ? (
